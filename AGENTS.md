...
3. **Loadout** — gear simulator. 10 equipment slots + 4 artifact slots, each with a substat editor. Items start at 3 substats; armor/accessories expand to 5, weapons expand to 6, artifacts stay at 3. The 5th/6th substat repeats a pool row. Multiple named sets supported with localStorage persistence, plus a per-stat delta comparison view. No "transfer to Calculator" button — stats from gear and base attributes intertwine and are not directly substitutable.
4. **Compare** — side-by-side build comparison. Also the only place with the full `BuildForm` (every attribute, gear flat/percent, target) and the complete `computeStats()` table, so it doubles as the stat sheet.
5. **Planner** — saved build presets.
6. **Essence** — read-only item substat reference. Nine pool cards (Melee/Ranged/Magic Weapon, Chest, Feet, Legs, Artifact, Accessory, Headgear) with roll rules, a base-attribute banner, and a computed reverse-lookup table. Data in `src/data/essenceData.ts`.
7. **Reference** — element chart, stances, status effects.
...
- `src/data/essenceData.ts` — nine item substat pools (34 rows, 78 options), `BASE_STAT_ROW`, `maxSubstats` counts (6 weapons / 5 armor-accessories / 3 artifacts), and `buildReverseIndex()`. Lives in its own file (not `gameData.ts`) because it is a large self-contained table. `EssenceOption.min` is optional; add minimums here once confirmed.
...
- **Loadout JSON export shape**: `{ app: "spiritcal", kind: "loadouts", version: 1, exportedAt: ISO string, sets: LoadoutSet[] }`. `parseLoadoutsFile` also accepts a bare array. Import is always additive — sets get fresh UUIDs so nothing is overwritten. Functions live in `storage.ts`: `exportLoadoutsToFile(sets)`, `parseLoadoutsFile(text)`.
